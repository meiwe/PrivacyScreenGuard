...
每个姿态组内逐维平均并 L2 归一化，形成多姿态加密模板。比对时取与全部模板的最大相似度——**主人扭头看侧屏时，侧脸会与侧脸模板匹配上**，从根上解决"侧脸被误判为陌生人"的问题（此前只能靠姿态过滤放过，存在防护空档）。侧脸样本缺失时仍可完成注册（弹窗确认），但扭头看侧屏可能误判。

采集画面仅在本地处理，不保存任何图像；落盘的只有 DPAPI 加密后的模板特征。
